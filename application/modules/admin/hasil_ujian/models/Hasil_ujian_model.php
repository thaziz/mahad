...
class Hasil_ujian_model extends CI_Model {
 ...
 public function ujian_ol($id){
    ...
    $this->db->select('*,u_id,u_start,DATE_ADD(u_start, INTERVAL u_waktu MINUTE) batas_akhir,timediff(DATE_ADD(u_start, INTERVAL u_waktu MINUTE),now()) as sisa_waktu');
    ...
    if($dx->num_rows()>0){
        ...
        /*expired time*/
        if(strtotime(date('H:i:s')) >=strtotime($dx->row()->ms_starttime) &&
           strtotime(date('H:i:s'))<=strtotime($dx->row()->ms_endtime)){
...
$this->db->select('*,SEC_TO_TIME(ms_waktu*60) sisa_waktu');
$this->db->join('db_subject','ms_matkul=id_subject');
$this->db->from('master_soal');
$d=$this->db->where('ms_id',$id)->get()->row(); 
...
function save_per_soal(){
    ...
    if($tipe=='pilihan'){
        ...
        return $this->db->update('ujian',array('u_nilai_pilihan' => $jumlah, ));
    ...
    if($tipe=='bs'){
        ...
        $this->db->where('u_id',$u_id);
        return $this->db->update('ujian',array('u_nilai_benarsalah' => $jumlah, ));
    ...
    if($tipe=='esai'){
        ...
        $this->db->where('sd_detailid',$_POST['ud_detailid']);
        $this->db->where('sd_kunci',$_POST['ud_jawaban']);
        $cek=$this->db->from('detail_soal_esai')->get();       
        if($cek->num_rows()>0){
            $_POST['ud_status']=1;
            $this->db->where('ud_master_soal',$_POST['ud_master_soal']);
            $this->db->where('ud_detailid',$_POST['ud_detailid']);
